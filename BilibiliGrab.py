...
def mylist_to_aid_list(mylist):
    """str/int->list"""
    request = urllib2.Request('http://www.bilibili.com/mylist/mylist-{mylist}.js'.format(mylist=mylist),
                              headers=FAKE_HEADER)
...
def find_cid_api(vid, p, cookies):
    ...
    if str(p) is '0' or str(p) is '1':
        ...
        biliurl = 'https://api.bilibili.com/view?appkey={APPKEY}&id={vid}&type=xml&sign={sign}'.format(APPKEY=APPKEY,
                                                                                                       vid=vid,
                                                                                                       SECRETKEY=SECRETKEY,
                                                                                                       sign=calc_sign(
                                                                                                           str2Hash))
    else:
        str2Hash = 'appkey={APPKEY}&id={vid}&page={p}&type=xml{SECRETKEY}'.format(APPKEY=APPKEY, vid=vid, p=p,
                                                                                  SECRETKEY=SECRETKEY)
        biliurl = 'https://api.bilibili.com/view?appkey={APPKEY}&id={vid}&page={p}&type=xml&sign={sign}'.format(
            APPKEY=APPKEY, vid=vid, SECRETKEY=SECRETKEY, p=p, sign=calc_sign(str2Hash))
...
def find_video_address_force_original(cid, header):
    ...
    request = urllib2.Request(
        api_url + 'appkey={APPKEY}&cid={cid}&sign={sign_this}'.format(APPKEY=APPKEY, cid=cid, SECRETKEY=SECRETKEY,
                                                                      sign_this=sign_this), headers=header)
    response = urllib2.urlopen(request)
    data = response.read()
...
def get_resolution(filename, probe_software):
    ...
    try:
        ...
        logging.debug('Software: {probe_software}, resolution {resolution}'.format(probe_software=probe_software,
...
def get_resolution_mediainfo(filename):
    ...
    [640,360]
    path to dimention"""
    resolution = str(os.popen('mediainfo \'--Inform=Video;%Width%x%Height%\' "' + filename + '"').read()).strip().split(
        'x')
...
def getvideosize(url, verbose=False):
    try:
        if url.startswith('http:') or url.startswith('https:'):
            ffprobe_command = ['ffprobe', '-icy', '0', '-loglevel', 'repeat+warning' if verbose else 'repeat+error',
                               '-print_format', 'json', '-select_streams', 'v', '-show_format', '-show_streams',
                               '-timeout', '60000000', '-user-agent', BILIGRAB_UA, url]
        else:
            ffprobe_command = ['ffprobe', '-loglevel', 'repeat+warning' if verbose else 'repeat+error', '-print_format',
                               'json', '-select_streams', 'v', '-show_streams', url]
...
def convert_ass_py3(filename, probe_software, resolution=[0, 0]):
    ...
    logging.info('Resolution is %dx%d' % (resolution[0], resolution[1]))
    if os.system('python3 %s/danmaku2ass3.py -o %s -s %dx%d -fs %d -a 0.8 -dm 8 %s' % (
            LOCATION_DIR, ass_name, resolution[0], resolution[1], int(math.ceil(resolution[1] / 21.6)), xml_name)) == 0:
...
def download_danmaku(cid, filename):
    ...
    os.system('curl -o "{filename}.xml" --compressed  http://comment.bilibili.com/{cid}.xml'.format(filename=filename,
                                                                                                    cid=cid))
...
def logcommand(command_line):
    logging.debug(
        'Executing: ' + ' '.join('\'' + i + '\'' if ' ' in i or '&' in i or '"' in i else i for i in command_line))
...
def main(vid, p, oversea, cookies, download_software, concat_software, is_export, probe_software, danmaku_only,
         time_fetch=5, download_thread=3):
    ...
    logging.info(
        '{vid_num} videos in part {part_now} to download, fetch yourself a cup of coffee...'.format(vid_num=vid_num,
                                                                                                    part_now=part_now))
    #Multi thread
    if len(rawurl) == 1:
...
if __name__ == '__main__':
    ...
    try:
        opts, args = getopt.getopt(argv_list, "ha:p:s:c:d:v:l:e:b:m:n:u:t:q:",
                                   ['help', "av=", 'part=', 'source=', 'cookie=', 'download=', 'concat=', 'log=',
                                    'export=', 'probe=', 'danmaku=', 'slient=', 'm3u=', 'mylist=', 'thread='])
    ...
    if IS_M3U == 1 and oversea not in {'0', '1'}:
        ...
        if IS_SLIENT == 0:
            ...
            if input_raw == 'q':
                ...
                oversea = input_raw
    concat_software, download_software, probe_software = check_dependencies(download_software, concat_software,
                                                                            probe_software)
    p_list = get_full_p(p_raw)
    if len(av_list) > 1 and len(p_list) > 1:
        logging.warning(
            'You are downloading multi parts from multiple videos! This may result in unpredictable outputs!')
        if IS_SLIENT == 0:
    ...
    BILIGRAB_HEADER = {'User-Agent': BILIGRAB_UA, 'Cache-Control': 'no-cache', 'Pragma': 'no-cache',
                       'Cookie': cookies[0]}
    if LOG_LEVEL == 'DEBUG':
        logging.debug(
        ...
        if str(p_raw) == '0':
            ...
            try:
                ...
                if IS_SLIENT == 0:
                    ...
                    p_list = [1]
            logging.info(
                'Your target download is av{vid}, part {p_raw}, from source {oversea}'.format(vid=vid, p_raw=p_raw,
                                                                                              oversea=oversea))
        for p in p_list:
            reload(sys)
            sys.setdefaultencoding('utf-8')
